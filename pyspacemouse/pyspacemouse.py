...
def open(
    ...
    if path is not None and device is not None:
        raise Exception("Cannot specify both device name and path. Pick only one.")
    if path is not None and DeviceNumber != 0:
        raise Exception(
            "Cannot specify both device path and DeviceNumber. Pick only one."
        )

    # The list of devices found matching the input criteria.
    # NOTE (peter): I think this approach is complicated. is there a simpler way?
    found_devices = []
    ...
    if device is None:
        if path is None:
            ...
            # Take the device that matches the path
            # This just finds the full device specs given the vendor and product ID
            device = get_device_by_vidpid()
            for device in all_devices:
                spec = device_specs[device]
                if dev.vendor_id == spec.hid_id[0] and dev.product_id == spec.hid_id[1]:
                    found_devices.append({"Spec": spec, "HIDDevice": dev})
                    print(f"{device} found, {spec=}")
